...
L = ['random', 'greedy', 'greedyDepth2', 'greedyValued', 'heuristicOneLevel', 'heuristicNlevels']
#DEBUG # print(L)
...
for val1 in L:
	for val2 in L:
		...
		for j in range(0,NOMBRE_ITERATIONS) :
			...
			import subprocess
			result = subprocess.run(['swipl', '-s', './ReversoMergedsansMC.pl', '-t' ,lanceur], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
